...
-- c)1)
-- em algebra relacional -> Cliente
SELECT Nome	
FROM Cliente;

-- c)2)
-- em algebra relacional -> pi nome (sigma localidade = 'Auada do Queixo' Cliente)
SELECT nome
FROM Cliente
WHERE localidade = 'Aguada do Queixo';

-- c)3)
-- em algebra relacional -> pi profissao Cliente
SELECT DISTINCT profissao
FROM Cliente;

-- c)4)
-- em algebra relacional -> ASC TAU designacao(pi designacao, preco Produto) 
SELECT designacao, preco
FROM produto
ORDER BY designacao;
-- ORDER BY assume que é ordenção ascendente  
-- ORDER BY designacao DESC -> se quisessemos por ordem decrescente

-- c)5) 
-- em algebra relcional -> pi numero, total (sigma data = '2017/10/05' Venda) 
SELECT numero, data, total
FROM Venda 
WHERE data = '2017/10/05';

-- c) ir à tabela somar os valores das vendas 'daquela' data) 
-- em algebra relcional -> numero gama sum (valor) (sigma data = '2017/10/05'(Venda join numero = venda VendaProduyto))   
SELECT v.numero, v.data, vp.produto, sum(vp.valor)
FROM Venda v, VendaProduto vp
WHERE data = '2017/10/05' and (v.numero = vp.venda);

-- d)1) 
INSERT INTO Cliente
VALUES(8, 'Lotupegui', '2017/10/29 11:11:11', 'Desempregado', 'P.B.D ponte', 'Madrid', '123', '123123', null, 0, null); 

-- d)2)
UPDATE Cliente SET Rua = 'Tenda 5X' WHERE Numero = '8';

-- d)3)
UPDATE Produto SET Preco = preco * 1.10 WHERE Tipo = 'Peixe';

-- d)4)
INSERT INTO Venda (6, '2017/10/29 11:11:11', 'S', 0, '8')
INSERT INTO VendaProduto(6, 8, 1, (SELECT preco FROM Produto WHERE numero = 8), 4.20);
INSERT INTO VendaProduto(6, 5, 1, (SELECT preco FROM Produto WHERE numero = 5), 1);
INSERT INTO VendaProduto(6, 10, 1, (SELECT preco FROM Produto WHERE numero = 10), 1);
INSERT INTO VendaProduto(6, 1, 1, (SELECT preco FROM Produto WHERE numero = 1), 1);

-- d)5)
SELECT c.nome, vp-produto, vp.quantidade, vp.preco
FROM Cliente c, Venda v, VendaProduto vp
WHERE v.numero = vp.venda and c.numero = v.cliente and c.numero = 8; 

DELETE FROM VendaProduto WHERE venda = 6;

DELETE FROM Venda WHERE cliente = 8;
-- 
-- ------------------------------------------------------
-- <fim>
-- Belo, O. 2017
